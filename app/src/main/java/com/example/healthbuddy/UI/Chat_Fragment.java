...
//            chatArrayAdapter.add(new ChatMessage(true, "Please Ask!"));
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (chatArrayAdapter.getItem(position).left) {
                    String url = chatArrayAdapter.getItem(position).getMessage().trim();

                    try {
                        int getIndex = url.indexOf("http");

                        if (getIndex > 0) {
                            String URL = url.substring(getIndex, url.length());
